...
Run this AFTER ``scripts/data/compute_norm_stats.py`` (needs norm stats for quantile normalization).
...
def main(config_name: str, n_bins: int = 128, max_frames: int | None = None) -> None:
    ...
    if base.norm_stats is None:
        raise RuntimeError(
            "norm_stats not found. Run scripts/data/compute_norm_stats.py first."
        )
