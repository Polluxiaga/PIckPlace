...
- **Policy 构建**：``get_config`` +
  ``policy_config.create_trained_policy`` + ``policy.infer``。
- **数据与归一化**：对齐 ``scripts/evaluation/eval.py`` —— checkpoint 下
  ``assets/<asset_id>/`` 的 norm stats；``--eval-repo-id`` 指向要读的 LeRobot 仓库
  （在 ``HF_LEROBOT_HOME`` 下）。
...
def _maybe_restore_tokenizer_assets_from_checkpoint(
    ...
    Mirrors the fallback used by ``scripts/evaluation/eval.py``. Useful when a
