...
def _rlbench_pick_place_repack(num_ctx_frames: int) -> dict:
    """Maps flat LeRobot keys from ``scripts/data/rlbench/convert_rlbench_to_lerobot.py`` to slash keys."""
    m: dict = {
        "observation/state": "state",
        "actions": "actions",
        "prompt": "prompt",
...
class LeRobotDROIDDataConfig(DataConfigFactory):
    ...
    To train on a custom DROID dataset, convert it to LeRobot format first.
    """
...
def _pickplace_all_qbin64_config(name: str) -> TrainConfig:
    return TrainConfig(
        name=name,
        model=pi0_fast.Pi0FASTConfig(
            ...
            fast_model_tokenizer_kwargs={
                ...
                "place_dim": 9,
                "n_bins": 64,
                "bin_edges_path": f"assets/{name}/pick_place_all/bin_edges.npy",
            },
        ),
        data=LeRobotRLBenchPickPlaceDataConfig(
            repo_id="minyangli/pick_place_all",
            assets=AssetsConfig(asset_id="pick_place_all"),
            num_ctx_frames=1,
...
def _pickplace_all_vq_config(name: str) -> TrainConfig:
    return TrainConfig(
        name=name,
        model=pi0_fast.Pi0FASTConfig(
            action_dim=18,
            ...
            fast_model_tokenizer_kwargs={
                ...
                "place_dim": 9,
                "codebook_size": 64,
                "codebook_path": f"assets/{name}/pick_place_all/codebook.npy",
            },
        ),
        data=LeRobotRLBenchPickPlaceDataConfig(
            repo_id="minyangli/pick_place_all",
...
_CONFIGS = [
    ...
    # For your own Aloha dataset, convert it to LeRobot format first.
    TrainConfig(
    ...
    TrainConfig(
        ...
        # For a custom DROID dataset, convert it to LeRobot format first.
