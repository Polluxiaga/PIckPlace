...
def _list_checkpoint_steps(checkpoint_root: epath.Path) -> list[int]:
    steps = []
...
def _run_post_train_eval_sweep(config: _config.TrainConfig) -> None:
    ...
    steps = _list_checkpoint_steps(checkpoint_root)
    if not steps:
    ...
    wandb.define_metric(checkpoint_step_key)
    wandb.define_metric(f"{metric_prefix}/*", step_metric=checkpoint_step_key)
    ...
    for step in steps:
        ...
        metrics, media = _evaluate_checkpoint(
            config,
...
def main(config: _config.TrainConfig):
    ...
    _run_post_train_eval_sweep(config)
